...
public class Player : Pawn {
    [JsonConstructor] public Player(Vector2i coords, MovementController movementController, Direction direction, int? health = 5) : base(new(coords.X * 64.0F, coords.Y * 64.0F), new(64.0f, 64.0f), TextureManager.Get(TextureType.Pawn, "player1"), health) {
        Health = health;
        Direction = direction;
    ...
    protected override void UpdatePosition(Vector2i lastCoords, Vector2i newCoords) {
        ...
        SoundManager.Play(SoundType.Move, "Light", Position / 64);
